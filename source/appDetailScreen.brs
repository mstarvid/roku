...
Function preShowDetailScreen(breadA=invalid, breadB=invalid) As Object
    ...
    if breadA = "7th Street Theater" then
        ...
        if type(msg) = "roSpringboardScreenEvent" then
            if msg.isScreenClosed()
                ...
                if msg.GetIndex() = 3
                    ...
                    ratingsUrl = showList[showIndex].contentRating
                
                    canvas.setLayer(0, { url: ratingsUrl, TargetRect: {x: 100, y: 100, w: 630, h: 420} })
                    canvas.Show()

                    while true
                        canvasmsg = wait(0, canvas.GetMessagePort())
                        if canvasmsg.isRemoteKeyPressed()  
                            if canvasmsg.GetIndex() = 6
                                canvas.Close()
                                exit while
                            endif    
                        endif
...
Function refreshShowDetail(screen As Object, showList As Object, showIndex as Integer) As Integer
    ...
    'screen.addbutton(3,"Content Ratings/Description")
...
Sub Boo()
    ...
    canvas = CreateObject("roImageCanvas")
    canvas.SetMessagePort( screen.GetMessagePort() )
    canvas.setLayer(0, { url: "http://mstarvid.com/stars.png", TargetRect: {x: 600, y: 300, w: 174, h: 365} })
    canvas.Show()
    while true
